...
CYB::API::String::UTF8 CYB::Platform::System::Path::LocateDirectory(const SystemPath ADirectory) {
	...
	case SystemPath::EXECUTABLE: {
		auto Module(MM.Call<Modules::Kernel32::GetModuleHandleW>(nullptr));
		if (Module != nullptr) {
			wchar_t Buffer[MAX_PATH];
			if (MM.Call<Modules::Kernel32::GetModuleFileNameW>(Module, Buffer, DWORD(MAX_PATH)) != 0)
				if (ADirectory == SystemPath::EXECUTABLE) {
					if (MM.Call<Modules::ShellAPI::PathRemoveFileSpecW>(Buffer) != 0) 
	...
	case SystemPath::TEMPORARY: {
		...
		{
			wchar_t Buffer[MAX_PATH];
			if (MM.Call<Modules::Kernel32::GetTempPathW>(Win32::DWORD(MAX_PATH), Buffer) == 0)
				throw Exception::SystemData(Exception::SystemData::SYSTEM_PATH_RETRIEVAL_FAILURE);
	...
	{
		wchar_t Buffer[MAX_PATH];
		if (MM.Call<Modules::Kernel32::GetCurrentDirectoryW>(Win32::DWORD(MAX_PATH), Buffer) == 0)
			throw Exception::SystemData(Exception::SystemData::SYSTEM_PATH_RETRIEVAL_FAILURE);
		return API::String::UTF16::ToUTF8(Buffer);
	}
	case SystemPath::USER:
	{
		...
		public:
			...
			{}
			~AutoFreeBuffer() {
				Core().FModuleManager.Call<Modules::Ole32::CoTaskMemFree>(FBuffer);
			}
		};
		wchar_t* Buffer;
		//https://msdn.microsoft.com/en-us/library/windows/desktop/dd378457(v=vs.85).aspx
		//GUID for roaming app data
		if (MM.Call<Modules::Shell::SHGetKnownFolderPath>(GUID{ 0xF1B32785, 0x6FBA, 0x4FCF, 0x9D, 0x55, 0x7B, 0x8E, 0x7F, 0x15, 0x70, 0x91 }, DWORD(0), nullptr, &Buffer) != S_OK)
			throw Exception::SystemData(Exception::SystemData::SYSTEM_PATH_RETRIEVAL_FAILURE);
...
void CYB::Platform::System::Path::DeleteDirectory(const API::String::UTF8& APath) {
	API::String::UTF16 As16(APath);
	if (Core().FModuleManager.Call<Modules::Kernel32::RemoveDirectoryW>(As16.WString()) == FALSE) {
...
void CYB::Platform::System::Path::DeleteFile(const API::String::UTF8& APath) {
	API::String::UTF16 As16(APath);
	auto& MM(Core().FModuleManager);
	if (MM.Call<Modules::Kernel32::DeleteFileW>(As16.WString()) == FALSE) {
		...
		case ERROR_ACCESS_DENIED: {
			...
			const auto Result(MM.Call<Modules::Kernel32::GetFileAttributesExW>(As16.WString(), GetFileExInfoStandard, &Attributes));
			if (Result != 0) {
...
void CYB::Platform::System::Path::Evaluate(API::String::UTF8& APath) {
	...
	if (Core().FModuleManager.Call<Modules::ShellAPI::PathCanonicalizeW>(OutputBuffer, As16.WString()) == FALSE)
...
bool CYB::Platform::System::Path::Verify(const API::String::UTF8& APath) const {
	...
	return Core().FModuleManager.Call<Modules::ShellAPI::PathFileExistsW>(Ref.WString()) == TRUE;
}
...
CYB::API::String::UTF8 CYB::Platform::System::Path::FullName(void) const {
	const auto Result(Core().FModuleManager.Call<Modules::ShellAPI::PathFindFileNameW>(const_cast<wchar_t*>(FWidePath.WString())));
	if (*Result == 0)
...
CYB::API::String::UTF8 CYB::Platform::System::Path::Name(void) const {
	const auto File(Core().FModuleManager.Call<Modules::ShellAPI::PathFindFileNameW>(const_cast<wchar_t*>(FWidePath.WString()))),
		Extension(Core().FModuleManager.Call<Modules::ShellAPI::PathFindExtensionW>(const_cast<wchar_t*>(FWidePath.WString())));
	if (*File == 0)
		throw Exception::SystemData(Exception::SystemData::STRING_VALIDATION_FAILURE);
	if (*Extension == 0)
	...
	FPathListing(nullptr)
{
	WIN32_FIND_DATA FindData;
	{
		...
		FFindHandle = Core().FModuleManager.Call<Modules::Kernel32::FindFirstFileW>(Query.WString(), &FindData);
	}
...
CYB::Platform::System::Implementation::Path::DirectoryEntry::~DirectoryEntry() {
	Core().FModuleManager.Call<Modules::Kernel32::FindClose>(FFindHandle);
}

void CYB::Platform::System::Implementation::Path::DirectoryEntry::operator++(void) {
	WIN32_FIND_DATA FindData;
	if (Core().FModuleManager.Call<Modules::Kernel32::FindNextFileW>(FFindHandle, &FindData) == 0)
