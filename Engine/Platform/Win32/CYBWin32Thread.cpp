...
CYB::Platform::Implementation::Thread::Thread(API::Threadable& AThreadable) :
	FThread(Core().FModuleManager.FK32.Call<ModuleDefinitions::KERNEL32::CreateThread>(nullptr, 0U, ThreadProc, &AThreadable, 0U, nullptr))
{
	if (FThread == nullptr) {
		//! @todo Log error
...
CYB::Platform::Implementation::Thread::~Thread() {
	Core().FModuleManager.FK32.Call<ModuleDefinitions::KERNEL32::CloseHandle>(FThread);
}
...
bool CYB::Platform::Thread::IsFinished(void) const {
	...
	return Core().FModuleManager.FK32.Call<ModuleDefinitions::KERNEL32::WaitForSingleObject>(FThread, 0U) == WAIT_OBJECT_0;
}

void CYB::Platform::Thread::Wait(void) const {
	Core().FModuleManager.FK32.Call<ModuleDefinitions::KERNEL32::WaitForSingleObject>(FThread, INFINITE);
}

void CYB::Platform::Thread::Sleep(const unsigned int AMilliseconds) {
	Core().FModuleManager.FK32.Call<ModuleDefinitions::KERNEL32::Sleep>(AMilliseconds);
}

void CYB::Platform::Thread::Yield(void) {
	Core().FModuleManager.FK32.Call<ModuleDefinitions::KERNEL32::SwitchToThread>();
}