...
void* CYB::Platform::System::VirtualMemory::InitReservation(const unsigned long long ANumBytes) {
	...
	auto const Reservation(reinterpret_cast<unsigned long long*>(Core().FModuleManager.Call<Modules::LibC::mmap>(nullptr, ANumBytes, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)));
...
CYB::Platform::System::VirtualMemory::~VirtualMemory() {
	API::Assert::Equal(Core().FModuleManager.Call<Modules::LibC::munmap>(FReservation, FReservationSize), 0);
}
...
void CYB::Platform::System::VirtualMemory::Commit(const unsigned long long ANumBytes) {
	if (ANumBytes > FCommitSize) {
		if(ANumBytes > FReservationSize || 
			Core().FModuleManager.Call<Modules::LibC::mprotect>(FReservation, ANumBytes, PROT_READ | PROT_WRITE) != 0)
...
void CYB::Platform::System::VirtualMemory::Access(const AccessLevel AAccessLevel) {
	if (Core().FModuleManager.Call<Modules::LibC::mprotect>(FReservation, FCommitSize, AAccessLevel == AccessLevel::READ_WRITE ? PROT_READ | PROT_WRITE : (AAccessLevel == AccessLevel::READ ? PROT_READ : PROT_NONE)) != 0)
...
void CYB::Platform::System::VirtualMemory::Discard(void* const AMemory, const unsigned long long ANumBytes) noexcept {
	...
	Core().FModuleManager.Call<Modules::LibC::madvise>(AMemory, ANumBytes, MADV_DONTNEED);
}