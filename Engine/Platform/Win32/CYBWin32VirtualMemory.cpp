	...
	if (ANumBytes >= PageSize) {
		...
		if (Difference < ANumBytes) {
			...
			if(TrueDiscardSize >= PageSize
				&& Core().FModuleManager.FK32Extended.Loaded<ModuleDefinitions::Kernel32Extended::DiscardVirtualMemory>())
					Core().FModuleManager.FK32Extended.Call<ModuleDefinitions::Kernel32Extended::DiscardVirtualMemory>(AlignedMemory, TrueDiscardSize);
		}
	}
}