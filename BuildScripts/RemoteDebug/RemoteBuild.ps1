...
if($LastExitCode -eq 0){
	...
	rm -r GDBSync
	$command = "cd $wd; cd ../..;cmake . -DCMAKE_BUILD_TYPE=DEBUG -DCOTIRE_MINIMUM_NUMBER_OF_TARGET_SOURCES=1;dos2unix Tests/llvm-gcov.sh;chmod +x Tests/llvm-gcov.sh;rm $(find -iname *.gcda);rm $(find -iname *.gcno);make clean;make && make test CTEST_OUTPUT_ON_FAILURE=1 && make Code.coveragexml"
