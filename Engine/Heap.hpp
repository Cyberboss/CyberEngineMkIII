...
namespace CYB {
	namespace API {
		...
		public:
			...
			*/
			virtual void* Alloc(const unsigned long long ASize) = 0;
			/*!
				@brief Allocate memory from the heap for use while preserving previous data
				...
				@par Exception Safety
					CYB::Exception::SystemData::MEMORY_COMMITAL_FAILURE if the heap does not have the space for the allocation and more system memory cannot be requested
			*/
			virtual void* Realloc(void* const APreviousAllocation, const unsigned long long ANewSize) = 0;
			/*!
