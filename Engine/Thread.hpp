...
namespace CYB {
	namespace API {
		...
		public:
			/*!
				@brief Called at the start of a threading operation in a new thread by the internal thread manager
				@par Calling Thread
				...
				@par Exception Safety
					This function will have exceptions throw from it logged at the DEV level and then ignored
			*/
			virtual void BeginThreadedOperation(void) = 0;
			/*!
				@brief Called by the internal thread manager to request cancellation of a thread. May also be called by the unit. When called the Threadable should do it's best to immediately return from BeginThreadedOperation
				@par Calling Thread
